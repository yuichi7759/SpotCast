...
import { metnoHourly, isOpenMeteoDown, markOpenMeteoDown, markOpenMeteoUp } from '@/lib/weather/metno'
...
async function buildForecast(lat: number, lng: number): Promise<HourlyWeather> {
  const url =
    ...
    if (isOpenMeteoDown()) return await metnoHourly(lat, lng)
    let d
    try {
      d = await fetchOpenMeteo(url)
      markOpenMeteoUp()
    } catch (e) {
      console.warn('[weather/hourly] Open-Meteo unavailable → MET Norway フォールバック', e)
      markOpenMeteoDown()
      return await metnoHourly(lat, lng)   // Open-Meteo障害時の予備プロバイダ
    }
    const city = ''
