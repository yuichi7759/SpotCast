    ...
    return (
      <div style={{ width: '100%', maxWidth: 400 }}>
        <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #e2e8f0', padding: '48px 32px', boxShadow: '0 8px 32px rgba(15,23,42,0.08)', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 900, color: '#0f172a', marginBottom: 10 }}>{t('auth.sentTitle')}</h2>
    ...
    <div style={{ width: '100%', maxWidth: 400 }}>
      <div style={{ background: '#fff', borderRadius: 20, border: '1px solid #e2e8f0', padding: '36px 32px', boxShadow: '0 8px 32px rgba(15,23,42,0.08)' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <h1 style={{ fontSize: 22, fontWeight: 900, color: '#0f172a', marginBottom: 6, letterSpacing: '-0.02em' }}>{t('auth.signup')}</h1>
          <p style={{ fontSize: 13, color: '#64748b' }}>{t('auth.signupHint')}</p>
        </div>
        ...
        <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          />
          <input
            type="password" placeholder={t('auth.pwMin')}
        ...
        <p style={{ textAlign: 'center', fontSize: 13, color: '#64748b', marginTop: 20 }}>
          {t('auth.haveAccount')}{' '}
          <Link href="/login" style={{ color: '#1d4ed8', textDecoration: 'none', fontWeight: 700 }}>{t('auth.login')}</Link>
        </p>
      </div>
    </div>
  )
